namespace pxsim.gpu {
    ...
    function drawTexturedTri(args: DrawTriArgs): void {
        ...
        const dstWidth = ops.toFixed(clamp(args.dst._width, 0, args.dst._width))
        const dstHeight = ops.toFixed(clamp(args.dst._height, 0, args.dst._height))
        const texWidth = ops.toFixed(clamp(args.tex._width, 0, args.tex._width))
        const texHeight = ops.toFixed(clamp(args.tex._height, 0, args.tex._height))
        ...
        for (; p.y <= pbounds.bottom; p.y += fxOne) {
            ...
            for (p.x = pbounds.left; p.x <= pbounds.right; p.x += fxOne) {
                if (w0 >= 0 && w1 >= w1Fudge && w2 >= 0) {
                    ...
                    if (color) {
                        ImageMethods.setPixel(args.dst, ops.toInt(p.x), ops.toInt(p.y), color)
                    }
                }
                w0 += A12
                w1 += A20
