...
interface Image {
    ...
    drawIcon(icon: Image, x: number, y: number, c: color): void;
...
namespace helpers {
    ...
    function _drawIcon(img: Image, icon: Image, xy: number, c: color): void { }

    function pack(x: number, y: number) {
        return (Math.clamp(-30000, 30000, x | 0) & 0xffff) | (Math.clamp(-30000, 30000, y | 0) << 16)
    }

    export function imageDrawIcon(img: Image, icon: Image, x: number, y: number, c: color): void {
