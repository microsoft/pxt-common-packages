...
} __attribute((__packed__)) __attribute__((aligned(4))) jd_serial_pre_t;
...
typedef struct {
    ...
    uint8_t service_arg;
    uint8_t data[0];
} __attribute((__packed__)) __attribute__((aligned(4))) jd_generic_packet_t;

typedef struct {
    jd_serial_pre_t serial;
    jd_generic_packet_t pkt;
    uint8_t data[0];
} jd_serial_header_t;
