...
#else
SoundOutputDestination soundOutputDestination = SoundOutputDestination::Pin;
#endif
...
void playTone(int frequency, int ms) {
    if (SoundOutputDestination::Speaker == soundOutputDestination) {
        ...
        if (NULL == pitchPin)
            pitchPin = lookupPin(PIN_PITCH_PIN);

        // no default pitch pin registered for this board
        if (NULL == pitchPin) {
            if (ms > 0)
                fiber_sleep(ms);
            return;
